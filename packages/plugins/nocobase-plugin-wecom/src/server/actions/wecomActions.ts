...
export const wecomActions = {
  getAuthUrl: async (ctx: Context, next: Next) => {
    ...
    const auth = (await app.authManager.get(authenticatorName, ctx)) as WecomAuth
    const redirectUri = `${getReqUrl(ctx.request)}api/community-wecom:redirectAuth?authenticator=${encodeURIComponent(authenticatorName)}&redirect=${encodeURIComponent(redirect || '')}`;
    ctx.body = auth.wecomApi.getLoginUrl(redirectUri)
